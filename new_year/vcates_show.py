...
def main():
    ...
    labels = ['improvment', 'compromise', 'keep']
    # label = 'Fixation duration & numerical attributes (red: better than all; blue: better than some; yellow: worse than all)'

    label = 'Fixation duration & category attributes'
    ...
    for i in range(1, 5):
        for j in range(1, 4):
            # print 531 + i * 3 + j
            ax = plt.subplot(4,3, 1 + (i-1) * 3 + j - 1)
            # axs.append(ax)
            ax.set_xlim([-6, 6])
            ax.set_ylim([-6, 6])
            # plt.sca(axs[i])
