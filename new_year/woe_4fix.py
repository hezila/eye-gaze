...
def main():
    ...
    print 'WOE'
    cm_labels = ['better than all', 'better than some', 'worse than all']
    fix_labels = ['high', 'middle', 'low', 'zero']
    num_count = 0.0
    for ct in ['=', '+', '-']:
    ...
    for ct in ['=', '+', '-']:
        ...
        for cl in [1, 2, 3]:
            ...
            ph = crit_counts[ct]['num']/num_count
    ...
    woe = woes['fix_cm']
    for cl in [1, 2, 3]:
        cl_count = sum([woe[fl][cl] for fl in [1, 2, 3, 4]]) + 0.0
        ph = cl_count/(38.0 * 7)
        for fl in [1, 2, 3, 4]:
            print 'woe(%s | %s) = ' % (cm_labels[cl-1], fix_labels[fl-1]),

            count = woe[fl][1] + woe[fl][2] +  woe[fl][3] + 0.0
            phe = woe[fl][cl]/count
